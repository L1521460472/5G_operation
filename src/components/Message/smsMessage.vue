        ...
        <el-tab-pane label="发送记录" name="first">
          <div class="headerBtn">
            ...
            <div class="headerBtnRight">
              <el-button size="small" class=" iconBtn active" style="margin-right:3px" v-has="'smsMessageAllBatchMessageExport'" @click="exportRepeat">
            ...
            <el-table border size="small" tooltip-effect="dark" v-loading="loading1" :data="sendData" :style="{'color':'#333','min-height': (minHeight || '') + 'px'}"
              ...
              <el-table-column label="操作" width="80" fixed="right">
                <template slot-scope="scope">
                  ...
                  <span class="operation" v-has="'smsMessageAllSendRecordPageList'" @click="toDetail(scope)">发送详情</span>
                  <!-- <el-tooltip class="item" effect="dark" content="发送详情" placement="top">
                    <img class="operation" src="../../assets/images/icon_24px_more.svg" v-has="'smsMessageAllSendRecordPageList'" @click="toDetail(scope)">
                  </el-tooltip> -->
                </template>
              </el-table-column>
            </el-table>
        ...
        <el-tab-pane label="接收记录" name="third">
          <div class="headerBtn">
            ...
            <div class="headerBtnLeft">
              ...
              <el-select v-model="status3" placeholder="状态" size="small" ref="statusSelect" @visible-change="statusChange" clearable @change="getReceiveList" style="margin-right:10px">
              ...
              <el-table border size="small" v-loading="loading3" tooltip-effect="dark" :data="receiveData" :style="{'color':'#333','min-height': (minHeight || '') + 'px'}"
                ...
                <el-table-column label="操作">
                  <template slot-scope="scope">
                    ...
                    <span class="operation" v-has="'smsMessageRecordAllView'" @click="handleDetails(scope.$index, scope.row)">查看记录</span>
                    <!-- <el-tooltip class="item" effect="dark" content="查看记录" placement="top">
                      <img class="operation" src="../../assets/images/icon_24px_detail.svg" v-has="'smsMessageRecordAllView'" @click="handleDetails(scope.$index, scope.row)" />
                    </el-tooltip> -->
                  </template>
                </el-table-column>
              </el-table>
...
import { sendBatch, sendDetailList, receiveList, recoedDetail, exportExcel } from '../../api/message/smsMessage'
...
export default {
  ...
  methods: {
    ...
    // 获取业务类型(废弃)
    getMessageTypeList() {
      getBusinessTypeList({id:2}).then(res => {
        if (res.status == 0) {
    ...
    async initData(){
      this.businessTypeList = await this.getBusiness()
