        ...
        <el-tab-pane label="发送记录" name="first">
          <div class="headerBtn">
            ...
            <div class="headerBtnLeft">
              ...
              <el-select v-model="enterpriseAccountAppId" placeholder="应用" size="small" ref="appSelect" @visible-change="appChange" clearable>
                <el-option v-for="item in appOptions" :key="item.id" :label="item.name" :value="item.id">
                </el-option>
              </el-select>
              <el-select v-model="sendStatus" placeholder="状态" size="small" ref="statusSelect" @visible-change="statusChange" clearable style="margin-right:10px">
              ...
              <el-date-picker size="small" v-model="time" type="daterange" :clearable="false" range-separator="~" format="yyyy-MM-dd" value-format="yyyy-MM-dd" start-placeholder="开始日期"
                end-placeholder="结束日期" style="height:36px;margin-right:10px">
              </el-date-picker>
            </div>
            <!-- 右侧菜单 -->
        ...
        <el-tab-pane label="发送详情" name="second">
          <div class="headerBtn">
            ...
            <div class="headerBtnLeft">
              ...
              <el-select v-model="status2" placeholder="状态" size="small" ref="statusSelect2" @visible-change="statusSelect2" clearable style="margin-right:10px">
...
import { getAppList } from '../../api/userName/api'
import { templeteAuditList } from '../../api/audit/api'
import { businessList } from '../../api/material/api'
        ...
        .then((res) => {
          if (res.status == 0) {
            ...
            data.map((item) => {
              ...
              // item.businessType = that.findAppByid(item.businessType, that.businessTypeList)
