  ...
  <div id="roleManagement" :style="{'height':innerHeight}" v-loading="loading"  element-loading-text="loading">
    ...
    <div class="content">
      ...
      <div class="roleRight">
        ...
        <el-button class="confirmJurisdiction" v-if="authorityEditBtn" type="primary" size="small" @click="confirmJurisdiction">确定权限</el-button>
      </div>
    </div>
    <!-- 新建或编辑角色 -->
    <el-dialog
...
<script>
import {getRoleData,addRole,editRole,deleteRole,getRoleAuthorityList,copyRole,editRoleAuthority,revertRoleAuthority} from '../../api/seeting/api'
    ...
    getAuthorityList(){
      ...
      getRoleAuthorityList(roleId,this.headers).then(res=>{
        ...
        if(res.status == 0){
          ...
          this.showRevertFlag = res.data.showRevertFlag
          this.checkedList = []
          res.data.treeNodeList.map(item=>{
            if(item.check){
