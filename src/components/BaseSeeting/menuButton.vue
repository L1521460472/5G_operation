    ...
    <div class="footer">
      <el-table
       ...
       >
        ...
        <el-table-column prop="type" show-overflow-tooltip label="类型" align="center" width="70">
          <template slot-scope="scope">
            <span v-if="scope.row.type == 1">菜单</span>
            <span v-if="scope.row.type == 2">按钮</span>
          </template>
        </el-table-column>
        <el-table-column prop="rank" show-overflow-tooltip label="排序" align="center" width="70"></el-table-column>
...
<script>
import {getAllMenuList,addMenu,getListMenu,getMenuButtonDetail,getDeleteDetail,deleteMenuButton,addButton,updateMenuButton} from '../../api/BaseSeeting/api'
...
export default {
    ...
    handleClick(row) {
      ...
      getMenuButtonDetail(params)
        .then((result) => {
          ...
          this.name = result.data.name; //简体名称
          this.route = result.data.url; //路由
          this.remarks = result.data.content; //描述
          this.code = result.data.code; //编号
          this.api = result.data.api; //API
          this.icon = result.data.icon //icon
          this.topMenu = result.data.parentId; //父id
          this.number = Number(result.data.menuRank); //排序
          this.data = result.data.menuRuleDetailVOList; //规则
        })
        .catch((err) => {
          console.error(err);
          this.$message({
  ...
  watch: {
    ...
    authorityBttonList:{
      handler(data) {
        console.log(getButtonList(data,'/menuButton'))
      },
      immediate: true,
      deep: true,
    }
